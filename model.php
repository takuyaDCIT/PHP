...
function check_session($str) {
	if (!isset($str)) {
		header('Location:../../htdocs/ecsite/redirect_to_login.php');
		exit();
	}	
}
...
function logout() {
	$_SESSION = [];
	session_unset();
	header('Location:../../htdocs/ecsite/logout.php');
	exit();
}
...
function get_db():PDO {
	try {
		...
		die("エラーメッセージ:get_db{echo $e -> getMessage()}");
		exit();
	}
	return $db;
}
...
function get_un_pw($db, $user_name) {
	try {
		...
		die("エラーメッセージ:get_un_pw{echo $e -> getMessage()}");
		exit();
	}
...
function check_same_user_name($db, $user_name) {
	try {
		...
		die("エラーメッセージ:check_same_user_name{echo $e -> getMessage()}");
		exit();
	}
...
function insert_into_item_table($db, $item_name, $price, $img_name, $status) {
	try{
		...
		die("エラーメッセージ:insert_into_item_table{echo $e -> getMessage()}");
		exit();
	}
}
...
function insert_into_stock_table($db, $item_id, $stock) {
	try {
		...
		die("エラーメッセージ:insert_into_stock_table{echo $e -> getMessage()}");
		exit();
	}
}
...
function update_stock_func($db, $changing_stock, $item_id) {
	try {
		...
		die("エラーメッセージ:update_stock_func{echo $e -> getMessage()}");
		exit();
	}
}
...
function update_status_func($db, $status, $item_id) {
	try {
		...
		die("エラーメッセージ:update_status_func{echo $e -> getMessage()}");
		exit();
	}
}
...
function delete_from_item_table($db, $item_id) {
	try {
		...
		die("エラーメッセージ:delete_from_item_table{echo $e -> getMessage()}");
		exit();
	}
}
...
function delete_from_stock_table($db, $item_id) {
	try {
		...
		die("エラーメッセージ:delete_from_stock_tabke{echo $e -> getMessage()}");
		exit();
	}
}
...
function get_item_list_mng($db) {
	try {
		...
		die("エラーメッセージ:get_item_list_mng{echo $e -> getMessage()}");
		exit();
	}
	return $result;
}
...
function insert_into_user_table($db, $user_name, $password_hashed) {
	try {
		...
		die("エラーメッセージ:insert_into_user_table{echo $e -> getMessage()}");
		exit();
	}
}
...
function get_un_cd($db) {
	try {
		...
		die("エラーメッセージ:get_un_cd{echo $e -> getMessage()}");
		exit();
	}
	return $result_uc;
}
...
function get_user_id($db, $user_name) {
	try {
		...
		die("エラーメッセージ:get_user_id{echo $e -> getMessage()}");
		exit();
	}
...
function get_item_list($db) {
	try {
		...
		die("エラーメッセージ:get_item_list{echo $e -> getMessage()}");
		exit();
	}
...
function get_item_already_in_cart($db, $item_id, $user_id) {
	try {
		...
		die("エラーメッセージ:get_item_already_in_cart{echo $e -> getMessage()}");
		exit();
	}
...
function add_into_cart($db, $user_id, $item_id, $amount) {
	try {
		...
		die("エラーメッセージ:add_into_cart{echo $e -> getMessage()}");
		exit();
	}
}
...
function check_amount_in_cart($db, $item_id, $user_id) {
	try {
		...
		die("エラーメッセージ:check_amount_in_cart{echo $e -> getMessage()}");
		exit();
	}
...
function check_stock($db, $item_id) {
	try {
		...
		die("エラーメッセージ:check_stock{echo $e -> getMessage()}");
		exit();
	}
...
function increase_amount_one($db, $item_id, $user_id) {
	try {
		...
		die("エラーメッセージ:update_amount{echo $e -> getMessage()}");
		exit();
	}
}
...
function get_amount_in_cart($db, $user_id) {
	try {
		...
		die("エラーメッセージ:get_amount_in_cart{echo $e -> getMessage()}");
		exit();
	}
...
function get_item_list_cart($db) {
	try {
		...
		die("エラーメッセージ:get_item_list_cart{echo $e -> getMessage()}");
		exit();
	}
	return $result_list;
}
...
function delete_from_cart($db, $item_id) {
	try {
		...
		die("エラーメッセージ:delete_from_cart{echo $e -> getMessage()}");
		exit();
	}
}
...
function update_on_purchase($db, $amount, $item_id) {
	try {
		...
		die("エラーメッセージ:update_on_purchase{echo $e -> getMessage()}");
		exit();
	}
}
...
function set_stock_zero($compare_status) {
	...
	for ($i = 0; $i < count($compare_status); $i++) {
		...
		try {
			...
			die("エラーメッセージ:set_stock_zero{echo $e -> getMessage()}");
			exit();
		}
	}
	$db = NULL;
...
function purchased_item_list($db, $user_id) {
	try {
		...
		die("エラーメッセージ:purchased_item_list{echo $e -> getMessage()}");
		exit();
	}
	return $result_list;
}
...
function update_stock_on_purchase($db, $item_id, $amount, $stock) {
	...
	$stmt -> execute();
		...
		die("エラーメッセージ:update_stock_on_purchase{echo $e -> getMessage()}");
		exit();
	}
}